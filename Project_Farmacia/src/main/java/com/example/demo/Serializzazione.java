	...
	public void Serialization() throws ParseException {   
		...
		try (BufferedReader br = new BufferedReader(new FileReader("ListaFarmacieLazio.csv"))) {
			...
			while ((line = br.readLine()) != null) {
				...
				v.add(new Farmacie_Lazio(values[1], values[2], values[3], Double.parseDouble(values[14]), values[4], values[5], values[6], values[7], values[8], values[9], values[10], values[11], Integer.parseInt(values[16]), Double.parseDouble(values[15]), Integer.parseInt(values[0]),date12,date13));
			}
			br.close();
		} catch (IOException i) {
			i.printStackTrace();
